...
local function notifyAdmins()
	for k,v in pairs(player.GetAll()) do
		if AdminsR[v:GetUserGroup()] then
			v:SendLua([[chat.AddText(Color(255,0,0),"A new report has been submitted by a user, please check the admin menu (!ra)")]])
		end
	end
end
...
hook.Add("PlayerSay","Handle vReport",function(ply, txt)
	...
	if txtTbl[1] and txtTbl[1] == "!report" then
		if txtTbl[2] then
			...
			ply:ChatPrint("Your report was sent to a member of staff. Please wait for someone to claim your report!")
			notifyAdmins()
		else
